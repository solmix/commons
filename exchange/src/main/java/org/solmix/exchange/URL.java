...
public class URL implements Comparable<URL> {
    ...
    public String decode(String input) {
        try {
            return builder.uRLDecoder.decode(input);
    ...
    public IPVersion getProtocolVersion() {
        return builder.iPVersion;
    }
    ...
    private void appendHostInfo(StringBuilder sb, boolean encoded, boolean withSlash) {
        ...
        if (!builder.host.isEmpty()) {
            ...
            if (builder.iPVersion != null) {
                switch (builder.iPVersion) {
                    case IPV6:
                    ...
                    default:
                        if (encoded) {
                            try {
                                String encodedHostName = URLEncoders.getRegNameEncoder(charset).encode(builder.host);
    ...
    private void appendPath(StringBuilder sb, boolean encoded) {
        URLEncoder pathEncoder = URLEncoders.getPathEncoder(charset);
        URLEncoder matrixEncoder = URLEncoders.getMatrixEncoder(charset);
    ...
    private void appendQuery(StringBuilder sb, boolean encoded, boolean withQuestionMark) {
        if (!builder.queryParams.isEmpty()) {
            ...
            URLEncoder queryParamEncoder = URLEncoders.getQueryParamEncoder(charset);
            while (it.hasNext()) {
            ...
            if (encoded) {
                try {
                    sb.append(URLEncoders.getQueryEncoder(charset).encode(builder.query));
                } catch (CharacterCodingException e) {
                    throw new IllegalArgumentException(e);
                }
            } else {
    ...
    private void appendFragment(StringBuilder sb, boolean encoded, boolean withHashSymbol) {
        if (!isNullOrEmpty(builder.fragment)) {
            ...
            if (encoded) {
                try {
                    sb.append(URLEncoders.getFragmentEncoder(charset).encode(builder.fragment));
                } catch (CharacterCodingException e) {
                    throw new IllegalArgumentException(e);
                }
            } else {
    ...
    public static class Builder {
        ...
        public Builder host(String host) {
            this.host = host;
            this.iPVersion = IPVersion.NONE;
            return this;
        }

        public Builder host(String host, IPVersion iPVersion) {
            this.host = host;
        ...
        public Builder resolveFromHost(String hostname) {
            ...
            try {
                ...
                host(inetAddress.getHostName(), inetAddress instanceof Inet6Address ?
                        IPVersion.IPV6 : inetAddress instanceof Inet4Address ?
                        IPVersion.IPV4 : IPVersion.NONE);
                return this;
                ...
                if (e.getMessage() != null && !e.getMessage().endsWith("invalid IPv6 address") &&
                    ...
                    try {
                        String idna = IDN.toASCII(uRLDecoder.decode(hostname));
                        host(idna, IPVersion.NONE);
    ...
    public static class Parser {
        ...
        private String parseUserInfo(Builder builder, String input)
            ...
            if (i > 0) {
                ...
                builder.userInfo(builder.uRLDecoder.decode(userInfo));
            }
            return remaining;
        }
        ...
        private void parsePathWithQueryAndFragment(Builder builder, String input)
            ...
            if (i >= 0) {
                builder.fragment(builder.uRLDecoder.decode(input.substring(i + 1)));
                ...
                if (!elem.isEmpty()) {
                    if (elem.charAt(0) == SEMICOLON_CHAR) {
                        ...
                        while (t != null) {
                            ...
                            builder.matrixParam(builder.uRLDecoder.decode(p.getFirst()),
                                    builder.uRLDecoder.decode(p.getSecond()));
                        ...
                        while (t != null) {
                            ...
                            if (i == 0) {
                                builder.pathSegment(builder.uRLDecoder.decode(segment));
                            } else {
        ...
        private void parseQuery(Builder builder, String query)
            ...
            while (s != null) {
                ...
                if (!isNullOrEmpty(param.getFirst())) {
                    builder.queryParam(builder.uRLDecoder.decode(param.getFirst()),
                            builder.uRLDecoder.decode(param.getSecond()));
                }
                s = p.getSecond();
            }
            if (builder.queryParams.isEmpty()) {
                builder.query(builder.uRLDecoder.decode(query));
            } else {
                builder.query(query);
            }
        }
