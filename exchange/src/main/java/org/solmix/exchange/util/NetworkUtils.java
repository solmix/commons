package org.solmix.exchange.util;
...
public class NetworkUtils {
    ...
    public static void extendSystemProperties() {
        ...
        try {
            ...
            for (NetworkInterface networkInterface : getAllRunningAndUpInterfaces()) {
                InetAddress inetAddress = getFirstNonLoopbackAddress(networkInterface, IPVersion.IPV4);
                if (inetAddress != null) {
    ...
    public static InetAddress getIPv4Localhost() throws UnknownHostException {
        return getLocalhost(IPVersion.IPV4);
    }

    public static InetAddress getIPv6Localhost() throws UnknownHostException {
        return getLocalhost(IPVersion.IPV6);
    }

    public static InetAddress getLocalhost(IPVersion ipversion) throws UnknownHostException {
    ...
    public static NetType getNetworkClass(InetAddress address) {
    ...
    public static boolean matchesNetwork(NetType given, NetType expected) {
        switch (expected) {
            case ANY:
                return EnumSet.of(NetType.LOOPBACK, NetType.LOCAL, NetType.PUBLIC, NetType.ANY)
                        .contains(given);
            case PUBLIC:
                return EnumSet.of(NetType.LOOPBACK, NetType.LOCAL, NetType.PUBLIC)
                        .contains(given);
            case LOCAL:
                return EnumSet.of(NetType.LOOPBACK, NetType.LOCAL)
                        .contains(given);
            case LOOPBACK:
                return NetType.LOOPBACK == given;
        }
        return false;
    }

    public static InetAddress getFirstNonLoopbackAddress(IPVersion ipversion) {
        InetAddress address;
    ...
    public static InetAddress getFirstNonLoopbackAddress(NetworkInterface networkInterface, IPVersion ipVersion) {
        if (networkInterface == null) {
            throw new IllegalArgumentException("network interface is null");
        }
        for (Enumeration<InetAddress> addresses = networkInterface.getInetAddresses(); addresses.hasMoreElements(); ) {
            InetAddress address = addresses.nextElement();
            if (!address.isLoopbackAddress() && (address instanceof Inet4Address && ipVersion == IPVersion.IPV4) ||
    ...
    public static InetAddress getFirstAddress(NetworkInterface networkInterface, IPVersion ipVersion) {
        if (networkInterface == null) {
            throw new IllegalArgumentException("network interface is null");
        }
        for (Enumeration<InetAddress> addresses = networkInterface.getInetAddresses(); addresses.hasMoreElements(); ) {
    ...
    public static boolean interfaceSupports(NetworkInterface networkInterface, IPVersion ipVersion) {
    ...
    public static IPVersion getProtocolVersion() {
    ...
    public static IPVersion findAvailableProtocols() {
        ...
        if ((host.startsWith("#") && host.endsWith("#")) || (host.startsWith("_") && host.endsWith("_"))) {
            ...
            if ("local".equals(host)) {
                ...
                if (host.toLowerCase(Locale.ROOT).endsWith(":ipv4")) {
                    return getFirstNonLoopbackAddress(IPVersion.IPV4);
                } else if (host.toLowerCase(Locale.ROOT).endsWith(":ipv6")) {
                    return getFirstNonLoopbackAddress(IPVersion.IPV6);
                } else {
                    return getFirstNonLoopbackAddress(getProtocolVersion());
                }
            } else {
                IPVersion networkProtocolVersion = getProtocolVersion();
                if (host.toLowerCase(Locale.ROOT).endsWith(":ipv4")) {
                    networkProtocolVersion = IPVersion.IPV4;
                    host = host.substring(0, host.length() - 5);
                } else if (host.toLowerCase(Locale.ROOT).endsWith(":ipv6")) {
                    networkProtocolVersion = IPVersion.IPV6;
                    host = host.substring(0, host.length() - 5);
                }
    ...
    public static InetAddress resolvePublicHostAddress(String host) throws IOException {
        ...
        if (address == null || address.isAnyLocalAddress()) {
            address = getFirstNonLoopbackAddress(IPVersion.IPV4);
            if (address == null) {
